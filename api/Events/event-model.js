...
function getAll() {
    ...
    const attendeeandfood = db('attendeeandfood as af').leftJoin('users as u', 'af.user_id', 'u.user_id').select('af.*', 'u.username')
...
function getByUserId(organizer_id) {
    return db('events').where({organizer_id})
